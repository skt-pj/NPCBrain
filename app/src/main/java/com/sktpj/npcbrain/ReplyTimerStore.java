...
final class ReplyTimerStore {
    ...
    synchronized ReplyTimerTask schedule(
        ...
        if (binding == null || !binding.isValid() || wakeAtMs <= 0L) return null;
        ...
        long createdAt = existing == null ? System.currentTimeMillis() : existing.createdAtMs;
        ReplyTimerTask updated = ReplyTimerTask.fromBinding(
                binding, jobId, wakeAtMs, reason, createdAt);
        ...
        if (!key.equals(ReplyTimerExecutionScope.currentSourceKey())) {
            ReplyTimerScheduler.schedule(appContext, updated);
        }
        return updated;
    }
