...
final class WorldKernelV210 {
    ...
    WorldCommitResultV210 commit(WorldCommandV210 command) {
        ...
        if (runner != null && result.committed) runner.runPending();
        return result;
    }
    ...
    private WorldCommitResultV210 commitLocked(WorldCommandV210 command) {
        ...
        try {
            ...
            long requestedTime = Math.max(currentTime, command.requestedTimeMs);

            if (WorldCommandV210.ADVANCE_TIME.equals(command.commandType)
            ...
            reduce(db, command, currentRevision, nextRevision, requestedTime, pending, commandResult);
            ...
            database.putMeta(db, WorldDatabaseV210.META_WORLD_TIME, Long.toString(requestedTime));
            database.putMeta(db, WorldDatabaseV210.META_NEXT_SEQUENCE, Long.toString(nextSequence));
            if (WorldCommandV210.ADVANCE_TIME.equals(command.commandType)) {
                database.putMeta(db, WorldDatabaseV210.META_LAST_ADVANCED, Long.toString(requestedTime));
            }

            try {
                commandResult.put("status", commandResult.optString("status", "committed"));
                commandResult.put("revision", nextRevision);
                commandResult.put("world_time_ms", requestedTime);
            ...
            db.setTransactionSuccessful();
            return new WorldCommitResultV210(true, false, nextRevision, requestedTime,
    ...
    private void reduce(
        ...
        if (WorldCommandV210.UPSERT_DUNGEON_STATE.equals(type)) {
            ...
            events.add(new PendingEvent(payload.optString("event_type", "dungeon_action"),
                    next.npcId(), array(payload.optJSONArray("participant_ids")), next.location(),
                    payload.optString("causation_id", ""), payload));
            return;
        }
        ...
        if (WorldCommandV210.APPLY_BRAIN_DECISION.equals(type)) {
            ...
            if (nextIntention != null) {
                ...
                try {
                    raw.put("intention", copy(nextIntention));
                    raw.put("state_version", current.stateVersion() + 1L);
                } catch (Exception ignored) {
                }
                database.upsertNpc(db, CanonicalNpcStateV210.fromJson(current.npcId(), raw), nextRevision);
            }
            return;
        }
    ...
    private static JSONArray participantsOf(String npcId) {
        JSONArray result = new JSONArray();
        if (npcId != null && !npcId.trim().isEmpty()) result.put(npcId.trim());
        return result;
    }
